...
function inscript_func_format_markdown ($args)
{
  if (isset ($args['text']))
  {
    global $wp_filter;
    ...
    if (!function_exists ('Markdown'))
      include (dirname (__FILE__).DIRECTORY_SEPARATOR."../markdown.php");
    $wp_filter = $backup;
    return Markdown ($args['text']);
  }
}

function inscript_func_format_textile ($args)
{
  if (isset ($args['text']))
  {
    global $wp_filter;
    ...
    if (!function_exists ('textile'))
      include (dirname (__FILE__).DIRECTORY_SEPARATOR."../textile1.php");
    $wp_filter = $backup;
    return textile ($args['text']);
  }
}
